...
describe('subscription formats', () => {
  it.each(targets)('converts base64 SS to %s native output', target => {
    ...
    if (target === 'shadowsocks') expect(JSON.parse(result.body)).toMatchObject({ version: 1, servers: [{ server: 'example.com', server_port: 443, method: 'aes-128-gcm', password: 'p:a:ss' }] });
    if (target === 'clash') expect(parse(result.body)).toMatchObject({ proxies: [{ name: 'Hong Kong', password: 'p:a:ss' }], rules: ['MATCH,PROXY'] });
    if (target === 'loon') expect(result.body).toBe('Hong Kong = Shadowsocks,example.com,443,aes-128-gcm,"p:a:ss"\n');
    if (target === 'quanx') expect(result.body).toBe('shadowsocks=example.com:443, method=aes-128-gcm, password=p:a:ss, tag=Hong Kong\n');
  });
  ...
  it.each(['auto', 'uri'] as InputType[])('parses Unicode, IPv6 and legacy SS with %s', type => {
    ...
    expect(serialize({ nodes, skipped: 0 }, 'quanx').body).toContain('[2001:db8::1]:8388');
  });
  ...
  it.each(['loon', 'quanx'] as const)('keeps WS TLS, SNI and password through %s roundtrip', target => {
    ...
    expect(again.nodes[0]).toMatchObject({ type: 'vmess', uuid, network: 'ws', tls: true, servername: 'tls.example.org', 'ws-opts': { path: '/ws', headers: { Host: 'cdn.example.org' } } });
  });
  ...
  it('handles all input formats and automatic detection', () => {
    const sources: [InputType, string][] = [
      ...
      ['sip008', '{"version":1,"servers":[{"remarks":"HK","server":"example.com","server_port":443,"method":"aes-128-gcm","password":"pass"}]}'],
  ...
  it('skips invalid and incompatible nodes, disambiguates names', () => {
    const parsed = parseSubscription([ss, ss, 'ss://invalid', `trojan://secret@example.org:443#Trojan`].join('\n'), 'uri');
    const out = serialize(parsed, 'shadowsocks');
    expect(out.count).toBe(2); expect(out.skipped).toBe(2);
  ...
  it('keeps advanced Clash fields and refuses lossy native conversion', () => {
    const parsed = parseSubscription(`proxies:\n - {name: reality, type: vless, server: example.com, port: 443, uuid: ${uuid}, tls: true, reality-opts: {public-key: test}, client-fingerprint: chrome}`, 'clash');
    expect(parse(serialize(parsed, 'clash').body).proxies[0]['reality-opts']).toEqual({ 'public-key': 'test' });
    expect(() => serialize(parsed, 'loon')).toThrow('NO_COMPATIBLE_NODES');
  });

  it('maps obfs plugins between SIP002, Loon, QuanX and SIP008', () => {
    const source = ss.replace('#Hong%20Kong', '/?plugin=obfs-local%3Bobfs%3Dtls%3Bobfs-host%3Dcdn.example.com#HK');
    ...
    expect(JSON.parse(sip).servers[0]).toMatchObject({ plugin: 'obfs-local', plugin_opts: 'obfs=tls;obfs-host=cdn.example.com' });
    expect(parseSubscription(sip, 'sip008').nodes[0]['plugin-opts']).toEqual({ mode: 'tls', host: 'cdn.example.com' });
  });
  ...
  it('rejects duplicate YAML keys, bad ports and unsupported URI options', () => {
    ...
    const parsed = parseSubscription(`vless://${uuid}@example.com:443?security=tls&type=ws&unknown=1`, 'uri');
...
describe('provider config', () => {
  it('supports JSON strings and JSON bindings with independent intervals', () => {
    const providers = { mysub: { url: 'https://example.com/sub', type: 'auto', minRefreshIntervalSeconds: 600 } };
    expect(getProvider(providers, 'mysub')).toEqual(getProvider(JSON.stringify(providers), 'mysub'));
    expect(getProvider(providers, 'mysub').minRefreshIntervalSeconds).toBe(600);
    expect(getProvider({ mysub: { url: 'https://example.com/sub' } }, 'mysub').minRefreshIntervalSeconds).toBe(300);
  });
  it.each(['http://example.com', 'https://localhost', 'https://127.0.0.1', 'https://[::1]', 'https://u:p@example.com', 'https://x.local'])('rejects unsafe upstream %s', url => {
  ...
  it('rejects prototype names, invalid intervals and header injection', () => {
    ...
    expect(() => getProvider({ a: { url: 'https://example.com', minRefreshIntervalSeconds: 0 } }, 'a')).toThrow();
    expect(() => getProvider({ a: { url: 'https://example.com', headers: { authorization: 'x\r\ny' } } }, 'a')).toThrow();
  });
});
