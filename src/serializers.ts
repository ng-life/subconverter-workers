...
import { AppError, record, str, type Dict, type Output, type Parsed, type ProxyNode, type Target } from './model';

const baseKeys = new Set(['name', 'type', 'server', 'port', 'cipher', 'password', 'uuid', 'username', 'alterId', 'network', 'tls', 'servername', 'sni', 'skip-cert-verify', 'udp', 'tfo', 'ws-opts', 'plugin', 'plugin-opts', 'protocol', 'protocol-param', 'obfs', 'obfs-param']);
const clashTypes = new Set(['ss', 'ssr', 'vmess', 'vless', 'trojan', 'http', 'socks5', 'hysteria2', 'tuic', 'hysteria', 'snell', 'wireguard']);
...
function transport(n: ProxyNode): { network: string; path: string; host: string } {
  ...
  if (Object.keys(ws).some(k => !['path', 'headers'].includes(k))) throw new Error('Unsupported websocket options');
  const headers = ws.headers === undefined ? {} : record(ws.headers);
  if (Object.keys(headers).some(k => k.toLowerCase() !== 'host')) throw new Error('Unsupported websocket headers');
  return { network, path: simple(ws.path || '/'), host: simple(headers.Host ?? headers.host ?? '') };
}
function plugin(n: ProxyNode): Dict {
  const p = record(n['plugin-opts'] ?? {});
  if (n.plugin === 'obfs') {
    if (!['http', 'tls'].includes(str(p.mode)) || Object.keys(p).some(k => !['mode', 'host'].includes(k))) throw new Error('Unsupported obfs');
  } else if (n.plugin === 'v2ray-plugin') {
    if ((p.mode && p.mode !== 'websocket') || Object.keys(p).some(k => !['mode', 'tls', 'host', 'path', 'mux'].includes(k)) || p.mux === true) throw new Error('Unsupported plugin');
...
function checkNative(n: ProxyNode, target: Target): void {
  ...
  if (!['ss', 'ssr', 'vmess', 'vless', 'trojan', 'http', 'socks5'].includes(n.type)) throw new Error('Unsupported type');
  ...
  const t = transport(n);
  if (t.network !== 'tcp' && !['vmess', 'vless'].includes(n.type)) throw new Error('Unsupported transport');
...
function loon(n: ProxyNode): string {
  ...
  const type = n.type === 'ss' ? 'Shadowsocks' : n.type === 'ssr' ? 'ShadowsocksR' : n.type === 'http' && n.tls ? 'https' : n.type;
  ...
  const p = plugin(n); const t = transport(n);
  if (n.plugin === 'obfs') { parts.push(`obfs-name=${simple(p.mode)}`); if (p.host) parts.push(`obfs-host=${simple(p.host)}`); }
  if (n.type === 'ssr') for (const key of ['protocol', 'protocol-param', 'obfs', 'obfs-param']) if (n[key]) parts.push(`${key}=${simple(n[key])}`);
  if (['vmess', 'vless'].includes(n.type)) {
    parts.push(`transport=${t.network}`, `over-tls=${bool(n, 'tls')}`);
    if (t.network === 'ws') { parts.push(`path=${t.path}`); if (t.host) parts.push(`host=${t.host}`); }
  }
  ...
  for (const [key, out] of [['udp', 'udp'], ['tfo', 'fast-open']]) if (n[key] !== undefined) parts.push(`${out}=${bool(n, key)}`);
...
function quanx(n: ProxyNode): string {
  ...
  const parts = [`${['ss', 'ssr'].includes(n.type) ? 'shadowsocks' : n.type}=${simple(host)}:${n.port}`];
  ...
  const p = plugin(n); const t = transport(n);
  if (n.plugin === 'obfs') { parts.push(`obfs=${simple(p.mode)}`); if (p.host) parts.push(`obfs-host=${simple(p.host)}`); }
  ...
  if (n.type === 'vmess') {
    if (t.network === 'ws') { parts.push(`obfs=${n.tls ? 'wss' : 'ws'}`, `obfs-uri=${t.path}`); if (t.host) parts.push(`obfs-host=${t.host}`); }
    else if (n.tls) parts.push('obfs=over-tls');
  } else if (['trojan', 'http', 'socks5'].includes(n.type)) parts.push(`over-tls=${bool(n, 'tls', n.type === 'trojan')}`);
  ...
  for (const [key, out] of [['udp', 'udp-relay'], ['tfo', 'fast-open']]) if (n[key] !== undefined) parts.push(`${out}=${bool(n, key)}`);
  parts.push(`tag=${simple(n.name)}`);
  return parts.join(', ');
}

function sip008(n: ProxyNode): Dict {
  if (n.type !== 'ss' || Object.keys(n).some(k => !['name', 'type', 'server', 'port', 'cipher', 'password', 'plugin', 'plugin-opts', 'udp', 'tfo'].includes(k))) throw new Error('Unsupported SS options');
  const result: Dict = { remarks: n.name, server: n.server, server_port: n.port, method: n.cipher, password: n.password };
  const p = plugin(n);
  if (n.plugin === 'obfs') {
    ...
    result.plugin_opts = Object.entries(p).filter(([k, v]) => k !== 'mode' && v !== false && v !== undefined).map(([k, v]) => v === true ? k : `${k}=${simple(v)}`).join(';');
  }
  return result;
}

export function serialize(parsed: Parsed, target: Target): Output {
  ...
  for (const n of parsed.nodes) {
    try {
      ...
      if (target === 'clash') {
        ...
        if (n.type === 'trojan' && n.servername) { proxy.sni = n.servername; delete proxy.servername; }
        values.push(proxy);
  ...
  if (target === 'clash') {
    body = stringify({ 'mixed-port': 7890, 'allow-lan': false, mode: 'rule', 'log-level': 'info', proxies: values, 'proxy-groups': [{ name: 'PROXY', type: 'select', proxies: values.map(v => (v as Dict).name) }], rules: ['MATCH,PROXY'] });
    contentType = 'application/yaml; charset=utf-8';
  } else if (target === 'shadowsocks') {
    body = JSON.stringify({ version: 1, servers: values }, null, 2) + '\n'; contentType = 'application/json; charset=utf-8';
  } else { body = values.join('\n') + '\n'; contentType = 'text/plain; charset=utf-8'; }
  if (new TextEncoder().encode(body).length > 1024 * 1024) throw new AppError(502, 'OUTPUT_TOO_LARGE');
