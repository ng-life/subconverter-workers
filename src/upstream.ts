...
export async function fetchSubscription(provider: Provider): Promise<{ body: string; userinfo: string | null }> {
  ...
  try {
    ...
    for (let redirects = 0; ; redirects++) {
      response = await fetch(url, { headers: { 'user-agent': 'subconverter-workers/1.0', ...provider.headers }, redirect: 'manual', signal: controller.signal });
      ...
      if (next.origin !== url.origin && Object.keys(provider.headers).length) throw new AppError(502, 'UPSTREAM_REDIRECT_ERROR');
      url = next;
    }
    if (!response.ok) throw new AppError(502, 'UPSTREAM_HTTP_ERROR');
    if (Number(response.headers.get('content-length')) > MAX_UPSTREAM_BYTES) throw new AppError(502, 'UPSTREAM_TOO_LARGE');
    ...
    throw new AppError(502, controller.signal.aborted ? 'UPSTREAM_TIMEOUT' : 'UPSTREAM_FETCH_FAILED');
  } finally {
    clearTimeout(timer);
