...
function common(n: Dict, o: Dict, native: 'loon' | 'quanx'): void {
  ...
  for (const [from, to] of Object.entries(map))
    if (o[from] !== undefined) n[to] = to === 'servername' ? o[from] : bool(o[from]);
...
function uri(line: string): Dict {
  ...
  if (scheme === 'vmess') {
    ...
    if (n.network === 'ws')
      n['ws-opts'] = { path: o.path || '/', headers: o.host ? { Host: o.host } : {} };
  ...
  if (scheme === 'ssr') {
    ...
    return {
      ...
      cipher: m[4],
      obfs: m[5],
      password: decode64(m[6]),
      name: q.has('remarks') ? decode64(q.get('remarks')!) : m[1],
      'protocol-param': q.has('protoparam') ? decode64(q.get('protoparam')!) : '',
      'obfs-param': q.has('obfsparam') ? decode64(q.get('obfsparam')!) : '',
    };
  }
  if (scheme === 'ss') {
    const [raw, fragment = ''] = line.slice(5).split('#');
    const [authority, query = ''] = raw.split('?');
    const full = authority.includes('@')
      ? authority.replace(/\/$/, '')
      : decode64(authority.replace(/\/$/, ''));
    const at = full.lastIndexOf('@');
    if (at < 0) throw new Error('Invalid SS');
    const user = full.slice(0, at);
    const [cipher, password] = splitFirst(
      user.includes(':') ? decodeURIComponent(user) : decode64(decodeURIComponent(user)),
      ':',
    );
    const n: Dict = {
      type: 'ss',
      ...endpoint(full.slice(at + 1)),
      cipher,
      password,
    ...
    if (q.has('plugin')) {
      ...
      for (const item of rest) {
        ...
        p[i < 0 ? item : item.slice(0, i)] = i < 0 ? true : item.slice(i + 1);
      }
  ...
  if (!['trojan', 'vless', 'hysteria2', 'hy2', 'tuic', 'http', 'https', 'socks5'].includes(scheme))
    throw new Error('Unsupported URI');
  const u = new URL(line);
  const q = u.searchParams;
  const n: Dict = {
    type: scheme === 'hy2' ? 'hysteria2' : scheme === 'https' ? 'http' : scheme,
    server: u.hostname,
    port: u.port || (scheme === 'http' ? 80 : 443),
    name: decodeURIComponent(u.hash.slice(1)),
  };
  if (scheme === 'vless' || scheme === 'tuic') n.uuid = decodeURIComponent(u.username);
  else if (['http', 'https', 'socks5'].includes(scheme))
    n.username = decodeURIComponent(u.username);
  else n.password = decodeURIComponent(u.username + (u.password ? ':' + u.password : ''));
  if (u.password && ['tuic', 'http', 'https', 'socks5'].includes(scheme))
    n.password = decodeURIComponent(u.password);
  ...
  if (n.network === 'ws')
    n['ws-opts'] = {
      path: q.get('path') || '/',
      headers: q.has('host') ? { Host: q.get('host') } : {},
    };
...
function loon(line: string): Dict {
  ...
  const type =
    ({ shadowsocks: 'ss', shadowsocksr: 'ssr', https: 'http' } as Record<string, string>)[
      kind.toLowerCase()
    ] || kind.toLowerCase();
  ...
  else if (['http', 'socks5'].includes(type) && tail.length && !tail[0].includes('=')) {
    n.username = tail.shift();
    n.password = tail.shift();
  }
  const o = opts(tail);
  common(n, o, 'loon');
  ...
  if (n.network === 'ws')
    n['ws-opts'] = { path: o.path || '/', headers: o.host ? { Host: o.host } : {} };
  if (type === 'ss' && o['obfs-name']) {
    n.plugin = 'obfs';
    n['plugin-opts'] = { mode: o['obfs-name'], host: o['obfs-host'] || '' };
  }
  if (type === 'ssr')
...
function quanx(line: string): Dict {
  ...
  const o = opts(tail);
  const n: Dict = {
    type: kind === 'shadowsocks' ? (o['ssr-protocol'] ? 'ssr' : 'ss') : kind,
    ...endpoint(address),
    name: o.tag,
  };
  ...
  if (n.type === 'ssr') {
    ...
    n['protocol-param'] = o['ssr-protocol-param'] || '';
    n.obfs = o.obfs || 'plain';
    n['obfs-param'] = o['obfs-host'] || '';
    ...
    if (n.type === 'ss') {
      ...
      n['ws-opts'] = {
        path: o['obfs-uri'] || '/',
        headers: o['obfs-host'] ? { Host: o['obfs-host'] } : {},
      };
    }
  } else if (n.type === 'ss' && o.obfs) {
    n.plugin = 'obfs';
  ...
  try {
    ...
    if (type === 'clash') {
      ...
      parser = (value) => {
        ...
        if (s.plugin) {
          const q = new URLSearchParams({
            plugin: str(s.plugin) + (s.plugin_opts ? ';' + str(s.plugin_opts) : ''),
          });
          return { ...uri(`ss://${btoa('x:x')}@example.com:443?${q}`), ...n };
        }
        return n;
      };
    } else {
      ...
      items = body
        ...
        .filter((line) => {
          if (/^\[.*\]$/.test(line)) {
            active = line.slice(1, -1).toLowerCase() === section;
            return false;
          }
          return (
            active &&
    ...
    return { schemaVersion: 1, nodes, skipped };
  } catch (e) {
